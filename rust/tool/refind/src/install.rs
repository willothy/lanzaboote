...
impl<S: Signer> Installer<S> {
    ...
    ///
    /// rEFInd is installed to the ESP if it doesn't exist or if it's not signed.
    fn install_refind(&self) -> Result<()> {
        let refind_efi = self
            .refind
            .join("share/refind/refind")
            .join(self.arch.refind_filename());

        let paths = [
            (&refind_efi, &self.esp_paths.efi_fallback),
            (&refind_efi, &self.esp_paths.refind_efi),
        ];
        ...
        for (from, to) in paths {
            ...
            let refind_is_signed = &self.signer.verify_path(to)?;
            if !refind_is_signed {
                log::warn!("{to:?} is not signed. Replacing it with a signed binary...");
            }

            if !to.exists() || !refind_is_signed {

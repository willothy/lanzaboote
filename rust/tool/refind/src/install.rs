...
impl InstallerBuilder {
    ...
    pub fn build<S: Signer>(self, signer: S) -> Installer<S> {
        ...
        let esp_paths = RefindEspPaths::new(self.esp, self.arch);
        ...
        Installer {
            ...
            refind: self.refind,
            refind_config_template: self.refind_config_template,
            refind_extra_config: self.refind_extra_config,
            refind_extra_files: self.refind_extra_files,
            signer,
...
impl<S: Signer> Installer<S> {
    pub fn install(&mut self) -> Result<()> {
        ...
        let generations = self.install_generations_from_links(&links)?;

        self.install_refind(&generations)?;
        ...
        log::info!("Successfully installed Lanzaboote with rEFInd.");
        Ok(())
    }
    ...
    /// Install rEFInd bootloader and generate configuration.
    fn install_refind(&self, generations: &[Generation]) -> Result<()> {
        ...
        if !refind_binary.exists() {
            return Err(anyhow!(
                "rEFInd binary not found at {:?}. Please ensure rEFInd is properly installed.",
                refind_binary
            ));
        }

        // Unlike systemd-boot, rEFInd does not embed a version in its binary, so there is no
        // cheap equivalent of `newer_systemd_boot` to skip redundant installs. Reinstall
        // unconditionally instead, which is what the rEFInd backend has always done.
        if let Some(pcrlock_paths) = &self.pcrlock_paths {
            ...
            lock_pe(&refind_binary, &bootloader_pcrlock)
                .context("Failed to lock rEFInd binary with systemd-pcrlock")?;
        }

        for to in [&self.esp_paths.efi_fallback, &self.esp_paths.refind_binary] {
            log::info!("Installing {}", to.display());
            install_signed(&self.signer, &refind_binary, to)
    ...
    /// Generate rEFInd configuration file content.
    fn generate_refind_config(&self, generations: &[Generation]) -> Result<String> {
        let mut config = String::new();

        // Add header
        config.push_str("# rEFInd configuration file generated by Lanzaboote\n");
        config.push_str("# Do not edit manually - changes will be overwritten\n\n");

        // Basic rEFInd settings
        config.push_str("timeout 20\n");
        config.push_str("use_nvram false\n");
        config.push_str("scanfor manual\n\n");

        // If there's a template, append it
        if let Some(template_path) = &self.refind_config_template {
